...
namespace IV.BaseUnity
{
	public class SetPivotWindow : EditorWindow
	{
		...
		private bool FrameOnSelect
		{
			get { return EditorPrefs.GetBool("Pivot_FrameOnSelect", false); }
			set { EditorPrefs.SetBool("Pivot_FrameOnSelect", value); }
		}
		...
		void OnGUI()
		{
			FrameOnSelect = EditorGUILayout.Toggle(new GUIContent("Frame On Select", "This will make the camera frame the new selected object when the selection changes"),
				FrameOnSelect);
			if (_transform)
			{
		...
		void SelectionChanged()
		{
			...
			if (_meshFilter != null && SetOnScene && FrameOnSelect)
			{
